...
class ModelCheckpoint:
    ...
    def set_mAP(self, value):
        """Set best mAP manually (used when loading from checkpoint)."""
        self.best_value = value

    def get_mAP(self):
        """Get best mAP for saving to checkpoint."""
    ...
    for batch_idx, (X, y) in enumerate(data_loader):
        ...
        if scaler:
            ...
            if torch.isnan(train_loss) or torch.isinf(train_loss):
                ...
                print("Loss value:", train_loss.item())
                raise RuntimeError("NaN or Inf in loss before backward()")
        ...
        # ✅ In từng batch
        print(f"  🔁 [Batch {batch_idx+1}/{total_batches}] - Loss: {train_loss.item():.4f}")

    avg_loss = sum(mean_train_loss) / total_batches
